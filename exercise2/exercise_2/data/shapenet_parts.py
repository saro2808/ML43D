...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def move_batch_to_device(batch, device):
        ...
        """
        batch['points'] = batch['points'].to(device).float()
        batch['segmentation_labels'] = batch['segmentation_labels'].to(device).long()
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        # Load point cloud and segmentation labels, subsample to 1024 points. Make sure points and labels still correspond afterwards!
        with open(ShapeNetParts.dataset_path / category_id / 'points' / f'{shape_id}.pts') as f:
            point_cloud = np.array([[float(f) for f in line.split()] for line in f.readlines()])
        with open(ShapeNetParts.dataset_path / category_id / 'points_label' / f'{shape_id}.seg') as f:
            labels = np.array([int(f) for f in f.readlines()])
        indices = np.random.choice(range(point_cloud.shape[0]), ShapeNetParts.num_points)
        
        # Important: Use ShapeNetParts.part_id_to_overall_id to convert the part labels you get from the .seg files from local to global ID as they start at 0 for each shape class whereas we want to predict the overall part class.
        ...
        labels = np.array([ShapeNetParts.part_id_to_overall_id[f'{category_id}_{label}'] for label in labels])
        return point_cloud[indices], labels[indices]