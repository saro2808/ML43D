...
class SimpleDataset(torch.utils.data.Dataset):
    def __init__(self, split):
        if 'train' == split:
            self.X_, self.y_ = generate_toy_data(4096)
        elif 'val' == split:
            self.X_, self.y_ = generate_toy_data(1024)
        else:
            raise ValueError("split should be either 'train' or 'val'.")

    def __getitem__(self, idx):
        return np.expand_dims(self.X_[idx], axis=0), self.y_[idx]
...
class SimpleModel(torch.nn.Module):
    def __init__(self):
        ...
        self.ffn = torch.nn.Linear(in_features=16, out_features=2)
    ...
    def forward(self, x):
        ...
        x = self.ffn(x.view((x.shape[0], x.shape[1])))
        return x
