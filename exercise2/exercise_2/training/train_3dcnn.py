...
def train(model, trainloader, valloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        for i, batch in enumerate(trainloader):
            ...
            input_data, target_labels = batch['voxel'], batch['label']
            ...
            for output_idx in range(prediction.shape[1]):
                loss_total += loss_criterion(prediction[:, output_idx, :], target_labels)  # Loss due to prediction[:, output_idx, :] (output_idx=0 for global prediction, 1-8 local)

            # compute gradients on loss_total (backward pass)
            gradient = loss_total.backward()
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    input_data, target_labels = batch_val['voxel'], batch_val['label']
                    ...
                    correct += torch.sum(predicted_label == target_labels).item()
                    for output_idx in range(prediction.shape[1]):
                        loss_total_val += loss_criterion(prediction[:, output_idx, :], target_labels).item()
