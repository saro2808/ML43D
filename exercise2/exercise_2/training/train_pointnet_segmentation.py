...
def train(model, trainloader, valloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        for i, batch in enumerate(trainloader):
            ...
            input_data, target_labels = batch['points'], batch['segmentation_labels']

            # zero out previously accumulated gradients
            optimizer.zero_grad()
            
            # forward pass
            prediction = model(input_data.transpose(1, 2)).transpose(1, 2)
            
            # compute total loss
            loss_total = loss_criterion(prediction, target_labels)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    input_data, target_labels = batch_val['points'], batch_val['segmentation_labels']
                    ...
                    loss_val += loss_criterion(prediction, batch_val['segmentation_labels']).item()
