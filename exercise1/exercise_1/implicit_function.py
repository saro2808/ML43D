...
def signed_distance_torus(x, y, z, R, r, x_0, y_0, z_0):
    ...
    xz = (x-x_0)**2 + (z-z_0)**2
    y = (y-y_0)**2
    return np.sqrt(xz + y + R**2 - 2 * R * np.sqrt(xz)) - r
    # ###############
...
def signed_distance_atom(x, y, z):
    ...
    return np.minimum(np.minimum(
        signed_distance_sphere(x, y, z, proton_radius, proton_center[0], proton_center[1], proton_center[2]),
        signed_distance_torus(x, y, z, orbit_radius, orbit_thickness, proton_center[0], proton_center[1], proton_center[2])),
        signed_distance_sphere(x, y, z, electron_radius, electron_center[0], electron_center[1], electron_center[2])
    )
    # ###############
