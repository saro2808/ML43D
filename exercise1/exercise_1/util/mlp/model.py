...
def signed_distance_mlp(x, y, z):
    checkpoint_path = pathlib.Path(__file__).parent / "checkpoint" / "_ckpt_epoch=349.ckpt"
    with safe_globals([ModelCheckpoint]):
        checkpoint = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    model = SimpleImplicitDecoder()
    state_dict = rename_state_dict(checkpoint["state_dict"], "model")
    # state_dict = rename_state_dict(torch.load((pathlib.Path(__file__).parent / "checkpoint" / "_ckpt_epoch=349.ckpt").absolute(), map_location="cpu", weights_only=True)["state_dict"], "model")
    model.load_state_dict(state_dict)
    model.eval()
    # Convert scalars to 1-element arrays if needed
    x = np.atleast_1d(x)
    y = np.atleast_1d(y)
    z = np.atleast_1d(z)
    ...
    sdf_values = []
    for points in tqdm(stacked_split):
        with torch.no_grad():
            sdf = model(points)
